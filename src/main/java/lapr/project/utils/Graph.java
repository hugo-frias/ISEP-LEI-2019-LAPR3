...
/**
 *
 * @author DEI-ESINF https://www.vogella.com/tutorials/JavaAlgorithmsDijkstra/article.html
 * @param <V>
 * @param <E>
 */
public class Graph<V, E> implements GraphInterface<V, E> {
    ...
    public Iterable<Edge<V, E>> edges() {
        ...
        for (V vert : vertices.keySet()) {
            for (V vert2 : vertices.get(vert).getAllAdjVerts()) {
                if (vert2 != null) {
                    edges.add(vertices.get(vert).getEdge(vert2));
                }
            }
        }
        return edges;
    ...
    public Iterable<Edge<V, E>> incomingEdges(V vert) {
        LinkedList<Edge<V, E>> edges = new LinkedList<>();
        for (V vertAux : vertices.keySet()) {
            for (V vertAux2 : vertices.get(vertAux).getAllAdjVerts()) {
                if (vertices.get(vertAux).getEdge(vertAux2).getVDest().equals(vert) && vertAux2 != null) {
                    edges.add(vertices.get(vertAux).getEdge(vertAux2));
                }
